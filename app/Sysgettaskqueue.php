...
class Sysgettaskqueue extends Common
{
	function __construct()
	{
		...
		if(!$this->systaskqueueDao)
		{
			$this->systaskqueueDao = new \model\Systaskqueue();
		}
	}
	function index($param = '')
	{
		...
		{
			//获取所有可用的任务队列信息
			$data_arr = $this->systaskqueueDao->get_all();
			...
			if(!empty($data) && isset($data['limit']) && $data['limit'] > 0)
			{
				...
				// $this->add_error_log('Sysgettaskqueue', $hardwareid, 0, '', '', time());
			}
		...
		$ip = $this->getIP();
		$this->add_error_log('sysgettaskqueue', $hardwareid, $ip, json_encode($param_arr), json_encode($return_param), $error_text);
	}
}
?>